...
namespace ColorBlast.Features
{
    ...
    /// </summary>
    public class EffectPipeline : IEffectScheduler
    {
        ...
        {
            // Order is critical: gravity fills gaps, spawner fills top,
            // checker updates group icons and validates match state.
            gridRefill.ApplyGravity();
            gridSpawner.SpawnNewCubeBlocks();
            gridChecker.CheckAllGrid();

            await UniTask.Yield(); // allow one frame for positions to settle before next update
        }
    }
}